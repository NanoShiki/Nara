...
{
	if (UGameUIManagerSubsystem* Manager = Cast<UGameUIManagerSubsystem>(GetOuter()))
	{
		return Manager;
	}
	UE_LOG(LogCommonGame, Error, TEXT("UGameUIPolicy::GetOwningUIManager: Failed to cast outer to UGameUIManagerSubsystem"));
	return nullptr;
}

UWorld* UGameUIPolicy::GetWorld() const
{
	if (auto* Manager = GetOwningUIManager())
	{
		if (auto* GameInstance = Manager->GetGameInstance())
		{
	...
	UE_LOG(LogCommonGame, Error, TEXT("Failed to get world"));
	return nullptr;
}
